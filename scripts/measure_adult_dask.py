...
# Function to check HDF5 file structure
def check_hdf5_file(path, key):
    try:
        with h5py.File(path, 'r') as file:
            keys = list(file.keys())
            if key not in keys:
                raise KeyError(f"Key '{key}' not found in HDF5 file. Available keys: {keys}")
    except Exception as e:
        print(f"Error checking HDF5 file: {e}")
        return False
    return True
...
for i in range(10):
    try:
        ...
        df = load_csv(path='../Datasets/adult.csv')
        sleep()
        df_json = load_json(path='../Datasets/adult.json')
        sleep()
        df_hdf = load_hdf(path='../Datasets/adult_dask.h5', key='a')
        sleep()
        
        if df_hdf is not None:
            save_csv(df_hdf, f'df_adult_dask{i}.csv')
            sleep()
            save_json(df_hdf, f'df_adult_dask{i}.json')
            sleep()
            save_hdf(df_hdf, f'df_adult_dask{i}.hdf', key='a')
            sleep()
